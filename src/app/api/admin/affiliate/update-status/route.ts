...
// --- Helper: marca estado de afiliado por email en BD (ruta API admin) ---
async function setAffiliateStatusByEmail(email: string, status: "approved" | "pending" | "rejected" | "suspended") {
  try {
    await fetch("/api/admin/affiliate/update-status", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ email, status }),
    });
  } catch {
    // silencioso; el refresh reintentará leer el estado
  }
}

// --- Helper: genera mailto de notificación de aprobación ---
function buildApproveMailto(v: Vendor, lang: "es" | "en") {
  const origin = typeof window !== "undefined" ? window.location.origin : "https://sommelierpro.ai";
  const subject =
    lang === "es" ? "Has sido aprobado — SommelierPro AI" : "You’ve been approved — SommelierPro AI";
  const body =
    (lang === "es"
      ? `Hola ${v.name},
...
Sigue estos pasos:
1) Crea tu cuenta de afiliado (Lemon): https://sommelierproai.lemonsqueezy.com/affiliates
2) Ingresa a tu Portal de Afiliados: ${origin}/dashboard-afiliado
3) En el paso 3, pega tu enlace de afiliado y comienza a referir.

Cualquier duda, responde este correo.

— Equipo SommelierPro AI`
      : `Hi ${v.name},

Your vendor account has been approved!

Next steps:
1) Create your affiliate account (Lemon): https://sommelierproai.lemonsqueezy.com/affiliates
2) Go to your Affiliate Portal: ${origin}/dashboard-afiliado
3) In step 3, paste your affiliate link and start referring.

If you have any questions, just reply to this email.

— SommelierPro AI Team`);

  return `mailto:${encodeURIComponent(v.email)}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
}

// --- Helper: genera mailto de notificación de rechazo ---
function buildRejectMailto(v: Vendor, lang: "es" | "en") {
  const subject =
    lang === "es" ? "Estado de solicitud — SommelierPro AI" : "Application status — SommelierPro AI";
  const body =
    (lang === "es"
      ? `Hola ${v.name},

Gracias por tu interés en ser vendedor/afiliado de SommelierPro AI.
Tras la revisión, en esta etapa no podremos aprobar tu solicitud.

Puedes volver a aplicar más adelante.

— Equipo SommelierPro AI`
      : `Hi ${v.name},

Thanks for your interest in becoming a SommelierPro AI vendor/affiliate.
After review, we cannot approve your application at this stage.
...
export default function SellersManager({ initialVendors = [], initialRequests = [] }: Props) {
  ...
  async function changeStatus(v: Vendor, s: VendorStatus) {
    ...
    // además actualizamos el estado de afiliado en BD (por email)
    if (s === "approved" || s === "rejected" || s === "pending" || s === "suspended") {
      await setAffiliateStatusByEmail(v.email, s);
    }
    router.refresh();
  }
  ...
  async function approveRequest(r: VendorRequest) {
    ...
    // marca aprobado el afiliado por email
    await setAffiliateStatusByEmail(r.email, "approved");
    router.refresh();
  }
  ...
  async function rejectRequest(r: VendorRequest) {
    ...
    await setAffiliateStatusByEmail(r.email, "rejected");
    router.refresh();
  }
  ...
  const t = {
    ...
    notify: lang === "es" ? "Notificar" : "Notify",
  };
                          ...
                          <td className="px-3 py-2 text-right">
                            <div className="inline-flex gap-2">
                              ...
                              <Button size="sm" variant="outline" asChild>
                                <a href={buildApproveMailto(v, lang)}>
                                  {t.notify}
                                </a>
                              </Button>
              ...
              <div className="rounded border p-4">
                ...
                <div className="mt-3 grid gap-2 text-sm">
                  ...
                  <div>
                    ...
                    <Badge variant={statusBadge(selected.status, lang).variant}>
                      {statusBadge(selected.status, lang).label}
                    </Badge>
                  </div>
                  <div>
                ...
                <div className="mt-4 flex gap-2">
                  ...
                  {/* Notificar por correo (detalle) */}
                  <Button variant="outline" asChild>
                    <a href={buildApproveMailto(selected, lang)}>
                      {t.notify}
                    </a>
                  </Button>
                    ...
                    <tr key={r.id} className="hover:bg-muted-30">
                      ...
                      <td className="px-3 py-2 text-right">
                        <div className="inline-flex gap-2">
                          ...
                          {/* Notificar aprobación directo desde solicitudes */}
                          <Button size="sm" variant="outline" asChild>
                            <a href={buildApproveMailto({ id: r.id, name: r.name, email: r.email, status: "approved", createdAt: r.createdAt }, lang)}>
                              {t.notify}
                            </a>
                          </Button>
                        </div>
