...
const AiResponseSchema = z.object({
  isAiGenerated: z.boolean().describe(
    'Set to true ONLY if you cannot find the specific wine and have to analyze a similar one.'
  ),
  ...
  country: z.string().optional().describe(
    "The country of origin. If the user does not provide it, you MUST research and provide it if the wine is known."
  ),
  wineryName: z.string().optional().describe(
    "The name of the winery. If the user does not provide it, you MUST research and provide it if the wine is known."
  ),
  notes: z.string().describe(
    "Your final expert opinion and conclusion. Comment on the wine's typicity, style, aging potential, and origin country. Maintain a warm, technical, and mentoring tone. This is your personal seal."
  ),
  corrections: z
    .array(
      z.object({
    ...
    .optional()
    .describe(
      "A list of corrections made to the user's input. ONLY report a correction if the user provided a non-empty value that was wrong. Do NOT report a correction if you are filling in a field the user left blank."
    ),
  pairingRating: z.number().min(1).max(5).optional().describe(
    'If foodToPair was provided, a rating from 1 to 5 for the pairing. Otherwise, null.'
  ),
  pairingNotes: z
    .string()
    .optional()
    .describe(
      'If foodToPair was provided, detailed notes explaining the pairing rating. Otherwise, null.'
    ),
  analysis: z
    .object({
      grapeVariety: z
        .string()
        .describe(
          "Crucial. The grape variety or a detailed blend composition (e.g., 'Cabernet Franc 77%, Cabernet Sauvignon 23%'). For blends, this is mandatory."
        ),
      wineryLocation: z
        .string()
        .optional()
        .describe(
          "The specific location/region of the winery (e.g., 'La Seca, Valladolid, España'). You MUST research and provide this if available."
        ),
      visual: z
        .object({
          description: z
            .string()
            .describe(
              "A rich, evocative visual description. Detail the color, hue, and reflections. Comment on the clarity (limpidity) and brightness. Describe the density of the legs (tears) and what it implies about the wine's body and alcohol content."
            ),
        })
      ...
      olfactory: z
        .object({
          description: z
            .string()
            .describe(
              'A complex olfactory analysis. Differentiate clearly between primary (fruit, floral), secondary (from fermentation/aging, e.g., vanilla, toast, butter), and tertiary (from evolution, e.g., leather, tobacco) aromas. Comment on the aromatic intensity and complexity.'
            ),
        })
      ...
      gustatory: z
        .object({
          description: z
            .string()
            .describe(
              "A thorough gustatory description. Describe the attack (initial impression), the evolution on the palate, and the finish. Detail the acidity, alcohol, body, and tannin structure. Explain how these elements are balanced and what the texture feels like (e.g., silky, astringent)."
            ),
        })
      ...
      recommendedPairings: z
        .string()
        .describe('Ideal food pairings for the wine, formatted as a numbered list with justifications.'),
      ...
      qualityLevel: z
        .string()
        .describe("The quality/commercial level of the wine (e.g., massive, standard, premium, icon)."),
      qualityRating: z
        .number()
        .min(1)
        .max(5)
        .describe('A numeric rating from 1 to 5 based on the quality level (1=massive, 5=icon).'),
      targetAudience: z.string().describe('Suggested expertise level, e.g., novice, intermediate, expert.'),
      appellation: z
        .string()
        .optional()
        .describe(
          "The wine's official appellation, including any special classifications (e.g., D.O. Rueda). You MUST research and provide this if available."
        ),
      barrelInfo: z
        .string()
        .describe(
          'Detailed information about barrel aging: time, percentage of different oaks, type of oak, and usage. THIS IS CRITICAL. For example, Amador Diez has barrel aging.'
        ),
      ...
      tanninLevel: z
        .enum(['Ligeros', 'Medios', 'Fuertes', 'Sin Taninos', 'Light', 'Medium', 'Strong', 'No Tannins'])
        .describe("Classification of the wine's tannin level."),
...
export async function analyzeWineFlow(
  ...
  if (!output.analysis) {
    ...
    if (
      analysisData.suggestedGlassType &&
      !/n\/?a|no especificado|not specified/i.test(analysisData.suggestedGlassType)
    ) {
    ...
    const getUrl = (res: PromiseSettledResult<any>) =>
      res.status === 'fulfilled' && (res as any).value?.media?.url
        ? (res as any).value.media.url
        : undefined;
