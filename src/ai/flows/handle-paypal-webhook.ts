...
const handlePaypalWebhookFlow = ai.defineFlow(
  ...
  async (payload) => {
    ...
    const userEmail = payload?.resource?.subscriber?.email_address as string | undefined;
    const subscriptionStatus = payload?.resource?.status as string | undefined;
    ...
    try {
      const db = adminDb(); // ✅ Firestore Admin
      ...
      // 2) Armar actualización
      const renewalDate = new Date();
      renewalDate.setMonth(renewalDate.getMonth() + 1); // PayPal maneja ciclo; simplificamos a +1 mes
      ...
      return { success: false, message: `Ocurrió un error inesperado al procesar el webhook de PayPal: ${e?.message || e}` };
    }
  }
);
