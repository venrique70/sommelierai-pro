...
export default function SellersManager({ initialVendors = [], initialRequests = [] }: Props) {
  ...
  async function createVendorReal() {
    ...
    setNewName(""); setNewEmail(""); setNewPhone(""); setNewDoc("");
    router.refresh();
  }
    ...
    <Card className="border">
      <CardHeader className="flex items-center justify-between gap-2 md:flex-row">
        <div>
          ...
          <CardDescription>Administra vendedores aprobados y solicitudes de ingreso.</CardDescription>
        </div>
        <div className="flex items-center gap-2">
          <Button variant="outline" onClick={refreshAll} disabled={loading}>
            {loading ? <>Cargando…</> : (<><RefreshCw className="mr-2 h-4 w-4" /> Refrescar</>)}
          </Button>
        ...
        {tab === "list" && (
          <div className="mb-4 grid gap-3 md:grid-cols-3">
            <div className="md:col-span-2">
              ...
              <Input placeholder="Nombre, email, ID / Pasaporte…" value={q} onChange={(e)=>setQ(e.target.value)} />
            </div>
            <div>
          ...
          <div className="space-y-6">
            ...
            {selected && (
              <div className="rounded border p-4">
                ...
                <div className="mt-4 flex gap-2">
                  ...
                  <Button variant="destructive" onClick={()=>removeVendor(selected)}>Eliminar</Button>
                </div>
              </div>
            )}
            ...
            <div className="rounded border p-4">
              ...
              <div className="grid gap-3 md:grid-cols-2">
                ...
                <Input placeholder="ID / Pasaporte Nº (opcional)" value={newDoc} onChange={(e)=>setNewDoc(e.target.value)} />
              </div>
